...
<body class="bg-slate-100 font-sans text-gray-800">
    ...
    <!-- JavaScript Logic -->
    <script>
        const DB_KEY = "cricket_portal_db_v7";
        ...
        function handleLogin() {
            ...
            // Wallet determination logic: Check userIDs or users data
            let walletBalance = 50;
            if (phone === "9569981484") {
                walletBalance = 5000;
            }

            // Agar admin ne pehle se is userid (jaise df43h) par points bhej rakhe hain
            ...
            // Save / Update both maps completely so they sync
            ...
            if (!db.userIDs[userid]) {
                ...
                db.users[phone].wallet = db.userIDs[userid].wallet; // sync back
            }
        ...
        function checkRecipientUser() {
            ...
            if (foundWallet !== null) {
                ...
                statusSpan.innerText = `ℹ️ ID not logged in yet. Points will be saved and credited automatically when they login with this ID!`;
            }
        }
        ...
        function adminSendPoints() {
            ...
            if (inputVal.length === 5) {
                // Unique User ID (jaise DF43H)
                if (!db.userIDs[inputVal]) {
                    db.userIDs[inputVal] = { phone: "Pending Login", wallet: 0 };
                }
                db.userIDs[inputVal].wallet += amount;
                
                // Agar ye ID kisi active user phone se linked hai toh uske phone wallet ko bhi turant update karo
            ...
            let totalWalletDisplay = inputVal.length === 5 ? db.userIDs[inputVal].wallet : db.users[inputVal].wallet;
